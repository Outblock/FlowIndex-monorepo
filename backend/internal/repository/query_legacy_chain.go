...
func recentAddressActivityFallbackWindowFromEnv() int64 {
	const defaultWindow int64 = 250000
	raw := strings.TrimSpace(os.Getenv("ACCOUNT_ACTIVITY_FALLBACK_WINDOW"))
	if raw == "" {
		return defaultWindow
	}
	v, err := strconv.ParseInt(raw, 10, 64)
...
// GetRecentTransactionsByAddressFallback supplements app.address_transactions with
// a recent raw-table scan. It is intentionally bounded to recent blocks so account
// pages stay usable when token/meta derivers fall behind near the head.
func (r *Repository) GetRecentTransactionsByAddressFallback(ctx context.Context, address string, limit, offset int) ([]models.Transaction, error) {
	if limit <= 0 {
		limit = 20
	}
	if offset < 0 {
	...
	window := recentAddressActivityFallbackWindowFromEnv()
	addrBytes := hexToBytes(address)
	if addrBytes == nil {
		return nil, nil
	}

	addressWithPrefix := "0x" + address
	rows, err := r.db.Query(ctx, `
		WITH tip AS (
			SELECT height
		...
		candidates AS (
			SELECT DISTINCT ON (s.block_height, s.transaction_id)
			       s.block_height, s.transaction_id
			FROM (
				SELECT t.block_height, t.id AS transaction_id
				FROM raw.transactions t
				CROSS JOIN tip
				WHERE t.block_height >= GREATEST(tip.height - $4::bigint, 0)
				  AND (
					t.payer_address = $1
					OR t.proposer_address = $1
					OR $1 = ANY(t.authorizers)
				  )

				UNION ALL

				SELECT e.block_height, e.transaction_id
				FROM raw.events e
				CROSS JOIN tip
				WHERE e.block_height >= GREATEST(tip.height - $4::bigint, 0)
				  AND (
					COALESCE(e.payload->>'from', '') IN ($2, $3)
					OR COALESCE(e.payload->>'to', '') IN ($2, $3)
					OR COALESCE(e.payload->>'address', '') IN ($2, $3)
					OR COALESCE(e.payload->>'owner', '') IN ($2, $3)
				  )
				  AND (
					e.type LIKE '%FungibleToken.Withdrawn%'
					OR e.type LIKE '%FungibleToken.Deposited%'
					OR e.type LIKE '%NonFungibleToken.Withdrawn%'
					OR e.type LIKE '%NonFungibleToken.Deposited%'
					OR e.type LIKE '%FLOWTokensWithdrawn%'
					OR e.type LIKE '%FLOWTokensDeposited%'
					OR e.type LIKE '%.TokensWithdrawn%'
					OR e.type LIKE '%.TokensDeposited%'
				  )
			) s
			ORDER BY s.block_height DESC, s.transaction_id DESC
			LIMIT $5 OFFSET $6
		)
		SELECT
			encode(t.id, 'hex') AS id,
		...
		ORDER BY c.block_height DESC, t.transaction_index DESC, c.transaction_id DESC
	`, addrBytes, address, addressWithPrefix, window, limit, offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
