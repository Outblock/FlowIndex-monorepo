        ...
        {user ? (
          ...
          <form onSubmit={handleSignIn} className="flex flex-col gap-1.5">
            <input
              ...
              className="w-full px-2.5 py-1.5 text-[11px] bg-white/[0.03] border border-white/10 rounded-sm text-zinc-200 placeholder-zinc-500 focus:outline-none focus:border-[var(--flow-green)]/40 transition-colors"
            />
            <button
              type="submit"
              disabled={authLoading}
              className="w-full flex items-center justify-center gap-2 px-3 py-1.5 text-[11px] font-bold text-zinc-400 uppercase tracking-widest border border-white/10 rounded-sm hover:text-white hover:border-[var(--flow-green)]/30 hover:bg-[var(--flow-green)]/5 transition-all disabled:opacity-50"
            >
              {authLoading ? "Sending..." : "Sign In"}
            </button>
            {authMessage && (
              <p className="text-[10px] text-zinc-400 text-center">{authMessage}</p>
            )}
          </form>
        )}
      </div>
