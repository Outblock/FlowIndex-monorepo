...
import { Loader2, ShieldCheck, Wallet, KeyRound, Check, Copy, ExternalLink, AlertCircle } from 'lucide-react';
import Avatar from 'boring-avatars';
import type { ProvisionState } from '../auth/usePasskeyWallet';

/** Derive 5 colors from an address (matches ConnectModal / AddressLink). */
...
interface PasskeyOnboardingModalProps {
  open: boolean;
  email?: string;
  onCreatePasskey: () => Promise<{ credentialId: string; publicKeySec1Hex: string }>;
...
export default function PasskeyOnboardingModal({
  ...
  const handleCreate = useCallback(async () => {
    ...
    setState('creating');
    try {
      const result = await onCreatePasskey();
  ...
  }, [onCreatePasskey, onProvisionAccounts, onPollTx, onSaveAddress]);
...
function NetworkRow({ label, status }: {
  label: string;
  status: { txId?: string; address?: string; error?: string; status: string };
}) {
  const explorerBase = label === 'Testnet'
    ? 'https://testnet.flowscan.io/account/'
    ...
    <div className="flex items-center gap-3 py-2 px-3 bg-zinc-800/50 border border-zinc-700/50 rounded-lg">
      <div className="w-5 h-5 flex items-center justify-center shrink-0">
        {status.status === 'sealed' && status.address ? (
          <Avatar size={20} name={`0x${status.address}`} variant="beam" colors={colorsFromAddress(status.address)} />
        ) : status.status === 'error' ? (
