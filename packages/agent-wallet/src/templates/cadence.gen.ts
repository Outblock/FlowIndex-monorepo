...
export type MetadataViewsMedia = any;
...
`;
  ...
  public async bridgeTokensFromEoaToFlowV3(rlpEncodedTransaction: number[], coinbaseAddr: string, vaultIdentifier: string, amount: string, recipient: string) {
    const code = `
import MetadataViews from 0xMetadataViews
import ViewResolver from 0xMetadataViews
import NonFungibleToken from 0xNonFungibleToken
...
`;
    let config = {
      cadence: code.trim(),
      name: "bridgeTokensFromEoaToFlowV3",
      type: "transaction",
      args: (arg: any, t: any) => [
...
`;
  ...
  // Tag: SrcTemplatesCadenceCollection
...
`;
  ...
  // Tag: SrcTemplatesCadenceHybridCustody
...
`;
  ...
  // Tag: SrcTemplatesCadenceLostAndFound
