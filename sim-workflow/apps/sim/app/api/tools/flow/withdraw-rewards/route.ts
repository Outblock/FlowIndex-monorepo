...
import { withdrawRewardsCadence } from '@/app/api/tools/flow/cadence-templates'

const logger = createLogger('FlowWithdrawRewards')
...
export async function POST(request: NextRequest) {
  try {
    ...
    const cadence = withdrawRewardsCadence(network)

    logger.info(`Withdrawing ${amount} rewards on ${network}`)
    ...
    logger.error('Failed to withdraw rewards', { error })
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to withdraw rewards',
      },
      { status: 500 }
    )
  }
