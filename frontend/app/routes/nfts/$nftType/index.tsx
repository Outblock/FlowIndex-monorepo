...
function NFTItemRow({ item, onClick }: { item: any; collectionVideoSupported: boolean; nftType: string; onClick: () => void }) {
  const nftId = item.nft_id || item.id || '';
  const name = item.name || `#${nftId}`;
  const thumb = extractImageUrl(item.thumbnail);
  const owner = item.current_owner || item.owner || item.address || '';
  const serial = item.serial_number;
  const rarity = item.rarity_description;
  ...
  return (
    <motion.tr
      ...
      className="border-b border-zinc-100 dark:border-white/5 group hover:bg-zinc-50 dark:hover:bg-white/5 transition-colors cursor-pointer"
      onClick={onClick}
    >
      <td className="p-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 flex-shrink-0 bg-zinc-100 dark:bg-zinc-800 overflow-hidden">
            {thumb ? (
              <img src={thumb} alt={name} className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
            ) : (
              <div className="w-full h-full flex items-center justify-center text-sm font-bold text-zinc-300 dark:text-zinc-600">
                {name.charAt(0).toUpperCase()}
              </div>
            )}
          </div>
      ...
      <td className="p-3">
        {owner ? (
          <AddressLink address={normalizeHex(owner)} prefixLen={8} suffixLen={4} />
        ) : (
      ...
      <td className="p-3">
        {serial ? (
          <span className="text-xs font-mono text-zinc-600 dark:text-zinc-400">{serial}</span>
        ) : (
          <span className="text-zinc-400">—</span>
        )}
      </td>
      <td className="p-3">
        {rarity ? (
          <span className="text-xs bg-zinc-100 dark:bg-white/10 px-2 py-0.5 text-zinc-600 dark:text-zinc-400">{rarity}</span>
        ) : (
          <span className="text-zinc-400">—</span>
        )}
      </td>
            ...
            {activeTab === 'nfts' && (
              <div>
                ...
                <div className="flex gap-2 mb-6">
                  ...
                  {/* View mode toggle */}
                  <div className="flex border border-zinc-200 dark:border-white/10 bg-white dark:bg-zinc-900">
                    <button
                ...
                {/* List view */}
                {!itemsLoading && items.length > 0 && viewMode === 'list' && (
                      ...
                      <table className="w-full text-left border-collapse">
                        <thead>
                          <tr className="border-b border-zinc-200 dark:border-white/5 bg-zinc-50/50 dark:bg-white/5">
                            <th className="p-3 text-xs font-semibold text-zinc-500 dark:text-gray-400 uppercase tracking-wider">Item</th>
                            <th className="p-3 text-xs font-semibold text-zinc-500 dark:text-gray-400 uppercase tracking-wider">Owner</th>
                            <th className="p-3 text-xs font-semibold text-zinc-500 dark:text-gray-400 uppercase tracking-wider">Serial</th>
                            <th className="p-3 text-xs font-semibold text-zinc-500 dark:text-gray-400 uppercase tracking-wider">Rarity</th>
                          </tr>
                        </thead>
                        <tbody>
                          <AnimatePresence mode="popLayout">
