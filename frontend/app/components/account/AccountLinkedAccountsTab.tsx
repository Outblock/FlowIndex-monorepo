...
function AccountNode({ data }: { data: any }) {
    ...
    const role = data.role as 'current' | 'parent' | 'child' | 'owned' | 'coa';
            ...
            <div className="flex items-start gap-3 mt-1">
                ...
                <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5 mb-0.5">
                        ...
                        {role === 'coa' && (
                            <span className="text-[8px] px-1.5 py-0.5 rounded bg-violet-500/10 text-violet-500 font-semibold uppercase tracking-wider">EVM · COA</span>
                        )}
                    </div>
                    {data.displayName && (
                    ...
                    {isCurrent ? (
                        ...
                        <AddressLink address={data.address} size={12} className="text-[10px] truncate" />
                    )}
                </div>
            </div>
...
export function AccountLinkedAccountsTab({ address }: Props) {
    const normalizedAddress = normalizeAddress(address);

    const [managerInfo, setManagerInfo] = useState<AccountManagerInfo | null>(null);
    const [ownedInfo, setOwnedInfo] = useState<AccountOwnedInfo | null>(null);
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            const result: AccountInfo = await cadenceService.getAccountInfo(normalizedAddress);
            const manager = result.manager;
            const owned = result.ownedAccount;
            setManagerInfo(manager);
            setOwnedInfo(owned);
            setCoaAddress(result.coaAddress || null);
            ...
            if (manager?.isManagerExists) {
                ...
                const children = manager.childAccounts || [];
            ...
            console.error('Failed to load linked accounts data', err);
            setError('Failed to load linked accounts data');
        } finally {
            setLoading(false);
        }
    }, [normalizedAddress]);
    ...
    useEffect(() => {
        if (!managerInfo && !ownedInfo && !coaAddress) return;
        ...
        const hasCoa = !!coaAddress;
        const totalChildren = children.length + ownedAccounts.length + (hasCoa ? 1 : 0);
        ...
        // Bottom row: COA node first, then HC children
        ...
        if (hasCoa) {
            const coaId = 'coa';
            const xOffset = (bottomIndex - (totalChildren - 1) / 2) * COL_WIDTH;
            newNodes.push({
                id: coaId,
                type: 'account',
                position: { x: centerX - 110 + xOffset, y: currentY + ROW_HEIGHT },
                data: {
                    address: `0x${coaAddress}`,
                    label: 'EVM Account',
                    role: 'coa',
                    isCurrent: false,
                },
        ...
        allChildren.forEach((c, i) => {
            const id = `child-${i}`;
            const xOffset = ((bottomIndex + i) - (totalChildren - 1) / 2) * COL_WIDTH;
        ...
        setNodes(newNodes);
        setEdges(newEdges);
    }, [managerInfo, ownedInfo, coaAddress, normalizedAddress, childMetadata, ftAccessibility, nftAccessibility, setNodes, setEdges]);
    ...
    const graphHeight = useMemo(() => {
        ...
        const children = ((managerInfo?.childAccounts as any[])?.length || 0) + ((managerInfo?.ownedAccounts as any[])?.length || 0) + (coaAddress ? 1 : 0);
        const rows = (parents > 0 ? 1 : 0) + 1 + (children > 0 ? 1 : 0);
        return Math.max(400, rows * 220 + 80);
    }, [managerInfo, ownedInfo, coaAddress]);
        ...
        <div>
            ...
            {!loading && !error && managerInfo !== null && ownedInfo !== null && !hasAnyRelationship && (
                <div className="flex flex-col items-center justify-center py-12 text-zinc-400 dark:text-zinc-600">
                    ...
                    <p className="text-xs uppercase tracking-widest">No linked accounts</p>
                    <p className="text-[10px] text-zinc-500 mt-1">This account has no parent, child, or EVM accounts</p>
                </div>
            )}
            ...
            {/* Summary info */}
            {!loading && (managerInfo || ownedInfo || coaAddress) && (
                <div className={`grid gap-4 mt-4 ${coaAddress ? 'grid-cols-3' : 'grid-cols-2'}`}>
                    ...
                    {coaAddress && (
                        <div className="border border-zinc-200 dark:border-white/5 p-3 bg-zinc-50 dark:bg-black/40 rounded-sm">
                            <div className="text-[9px] uppercase tracking-widest text-zinc-500 mb-1">EVM (COA)</div>
                            <div className="text-sm font-mono font-bold text-violet-600 dark:text-violet-400 truncate">
                                0x{coaAddress}
                            </div>
                            <div className="text-[10px] text-zinc-500 mt-1">Cadence Owned Account</div>
                        </div>
                    )}
                </div>
            )}
