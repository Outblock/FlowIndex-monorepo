...
/** Return ALL applicable activity badges for the tx detail page (not just the first match). */
export function deriveAllActivityBadges(tx: any): { type: string; label: string; color: string; bgColor: string }[] {
    const tags: string[] = (tx.tags || []).map((t: string) => t.toUpperCase());
    const badges: { type: string; label: string; color: string; bgColor: string }[] = [];
    const seen = new Set<string>();

    const add = (b: { type: string; label: string; color: string; bgColor: string }) => {
        if (!seen.has(b.label)) { seen.add(b.label); badges.push(b); }
    };

    // Map known tags to badges
    const tagStyles: Record<string, { type: string; label: string; color: string; bgColor: string }> = {
        ...
        ACCOUNT_CREATED:   { type: 'account', label: 'New Account', color: 'text-cyan-600 dark:text-cyan-400', bgColor: 'border-cyan-300 dark:border-cyan-500/30 bg-cyan-50 dark:bg-cyan-500/10' },
        KEY_UPDATE:        { type: 'key', label: 'Key Update', color: 'text-orange-600 dark:text-orange-400', bgColor: 'border-orange-300 dark:border-orange-500/30 bg-orange-50 dark:bg-orange-500/10' },
        ...
        CONTRACT_ADDED:    { type: 'deploy', label: 'Deploy', color: 'text-blue-600 dark:text-blue-400', bgColor: 'border-blue-300 dark:border-blue-500/30 bg-blue-50 dark:bg-blue-500/10' },
        CONTRACT_UPDATED:  { type: 'deploy', label: 'Contract Update', color: 'text-blue-600 dark:text-blue-400', bgColor: 'border-blue-300 dark:border-blue-500/30 bg-blue-50 dark:bg-blue-500/10' },
    ...
    // Fallback: if no tags produced badges, use deriveActivityType
    if (badges.length === 0) {
