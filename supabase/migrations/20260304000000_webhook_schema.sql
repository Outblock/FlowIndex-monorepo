...
-- Creates tables for rate limit tiers, user profiles, API keys, endpoints,
-- subscriptions, delivery logs, and workflows.
...
-- teams and team_memberships already exist from earlier migrations;
...
DROP POLICY IF EXISTS users_own_keys ON public.api_keys;
CREATE POLICY users_own_keys ON public.api_keys FOR ALL USING (auth.uid() = user_id);

DROP POLICY IF EXISTS users_own_endpoints ON public.endpoints;
...
DROP POLICY IF EXISTS users_own_logs ON public.delivery_logs;
...
DROP POLICY IF EXISTS users_own_workflows ON public.workflows;
CREATE POLICY users_own_workflows ON public.workflows FOR ALL USING (auth.uid() = user_id);
...
-- Backfill: ensure every existing user has a profile + personal team
